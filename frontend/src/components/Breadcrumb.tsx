...
export default function Breadcrumb() {
  ...
  return (
    <nav className="border-y py-3 text-sm text-gray-500 w-full mx-auto bg-white dark:bg-black/20 " data-testid="breadcrumb-nav">
      <div className="lg:max-w-400 px-4 mx-auto" data-testid="breadcrumb-container">
        <Link href="/" data-testid="breadcrumb-home-link">首頁</Link>
        {segments.map((seg, idx) => {
          ...
          return (
            <span key={href} data-testid={`breadcrumb-segment-${idx}`}>
              <span className="mx-2" data-testid={`breadcrumb-separator-${idx}`}> {" / "} </span>
              <span className="text-blue-400" data-testid={`breadcrumb-label-${idx}`}>
                {displayLabel}
              </span>
