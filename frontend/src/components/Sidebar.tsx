...
const sidebar = ({ isOpen, onClose }: SidebarProps) => {
    return (
        <div className={`sideBar fixed bg-white dark:bg-black/50 w-full h-full z-10 flex flex-col gap-12 overflow-auto left-0 px-6 py-6 backdrop-blur-sm lg:hidden transition-all duration-300 ease-in-out ${
            ...
            <div className="mt-30" data-testid="sidebar-content">  
                {/* <Link href="/login" className="mb-10">登入</Link> */}
                <div className="flex flex-col gap-3" data-testid="sidebar-links">
                    {sidebarLink.map((component, index) => (
                        component.name ? <Link key={index} className="text-2xl font-medium" href={component.link} onClick={onClose} data-testid={`sidebar-link-${index}`}>{component.name}</Link> : ''
                    ))}
                </div>
